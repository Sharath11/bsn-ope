...
func isNodeAddedEvent(logs *types.Log) bool {
	if len(logs.Topics) > 0 {
		return bytes.Equal(logs.Topics[0].Bytes(), CensorshipContractNodeAddedEventTopicHash.Bytes())
	} else {
		return false
	}
}

func isUpdateCensorshipContractEvent(logs *types.Log) bool {
	if len(logs.Topics) > 0 {
		return bytes.Equal(logs.Topics[0].Bytes(), UpdateCensorshipContractEventTopicHash.Bytes())
	} else {
		return false
	}
}
